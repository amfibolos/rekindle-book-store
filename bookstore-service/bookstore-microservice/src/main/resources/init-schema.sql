...
DROP MATERIALIZED VIEW IF EXISTS bookstore.order_bookstore_m_view;

CREATE MATERIALIZED VIEW bookstore.order_bookstore_m_view
...
AS
 SELECT r.id AS bookstore_id,
  ...
  WHERE r.id = rp.bookstore_id AND p.id = rp.product_id
WITH DATA;
...
CREATE OR replace function bookstore.refresh_order_bookstore_m_view()
returns trigger
AS '
BEGIN
    refresh materialized VIEW bookstore.order_bookstore_m_view;
